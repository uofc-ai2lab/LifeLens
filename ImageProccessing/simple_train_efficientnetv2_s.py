...
def create_model(num_classes: int, pretrained: bool = True) -> nn.Module:
    # Use a variant with available pretrained weights in timm
    # Alternatives: "tf_efficientnetv2_s" (TF port) or "efficientnetv2_rw_s"
...
def main():
    parser = argparse.ArgumentParser(description="Simple training using folder structure (single-label) - EfficientNetV2-S")
    ...
    checkpoint_path = os.path.join("experiments/checkpoints/simple", f"best_tf_efficientnetv2_s.pt")
    ...
    try:
        ...
        metrics_path = os.path.join("experiments", "checkpoints", "simple", "metrics_tf_efficientnetv2_s.json")
