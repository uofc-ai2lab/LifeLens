...
def main():
    ...
    if set(ds_classes) != set(ckpt_classes):
        msg = (
            "Class names differ between dataset and checkpoint.\n"
            f"Dataset classes: {ds_classes}\n"
            f"Checkpoint classes: {ckpt_classes}\n"
            "Use the same dataset used to train the checkpoint."
        )
    ...
    with torch.no_grad():
        for images, targets in loader:
            ...
            for i in range(images.size(0)):
                ...
                caption = f"pred: {pred_name} ({conf:.2f}) | true: {true_name} | {mark}"
