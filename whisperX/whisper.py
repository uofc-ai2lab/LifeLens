...
async def align_whisper_output(device: str, audio_file: str, result: dict, audio: numpy.ndarray):
...
def assign_speakers(device: str, audio_file: str, result: dict, USE_OFFLINE_MODELS: int, HUGGING_FACE_TOKEN: str):
...
async def run_whisperx():
    ...
    # 2. Align whisper output
    alignment_result = await align_whisper_output(device, audio_file, transcription_result, audio)
    ...
    if (HUGGING_FACE_TOKEN is None) or (HUGGING_FACE_TOKEN == ""):
        raise ValueError("HUGGING_FACE_TOKEN environment variable not set. Please set it to your Hugging Face token.")
    USE_OFFLINE_MODELS = int(os.getenv("USE_OFFLINE_MODELS", "0"))
    diarize_result = assign_speakers(device, audio_file, alignment_result, USE_OFFLINE_MODELS, HUGGING_FACE_TOKEN)
