...
# ═══════════════════════════════════════════════════════════════════════════
# POST-CAMERA PIPELINE
# Runs once after the camera session ends, across all accumulated snapshots
# ═══════════════════════════════════════════════════════════════════════════

def run_post_camera_pipeline(settings: Dict[str, Any], snapshot_count: int) -> bool:
    """
    Runs the full downstream pipeline on all snapshots captured during
    the camera session. Called once after the camera loop exits.

    Args:
        settings:       Video pipeline settings dict.
        snapshot_count: Number of snapshots captured — logged for reference.
    Returns:
        True if pipeline completed without critical failure.
    """
    if snapshot_count == 0:
        log.warning("No snapshots captured during camera session — skipping pipeline")
        return False
...
def main(video_pipeline: Optional[GStreamerVideoPipeline] = None) -> int:
    ...
    # ── Snapshot counter ─────────────────────────────────────────────────
    # Tracks how many ReID snapshots were captured during the camera session.
    # Passed to run_post_camera_pipeline after the loop exits.
    snapshot_count = 0

    # ── ReID service ─────────────────────────────────────────────────────
    def _on_reid(event: ReIDEvent):
        """
        Callback fired on the video loop thread when primary person is matched.
        Saves the snapshot — processing happens after camera ends.
        """
    ...
    log.header("Camera session started  —  press Q or ESC to end and run pipeline")

    # ═════════════════════════════════════════════════════════════════════
    # CAMERA PHASE
    # ReID runs here. No downstream processing until this loop exits.
    # ═════════════════════════════════════════════════════════════════════
    try:
        while True:
            ...
            if detected:
                if not enrolled:
                    # Enrollment phase — buffer body embeddings via YuNet + ResNet50
                    if reid.enroll_from_frame(frame):
                        enrolled  = True
                        last_snap = now
                        log.success("Primary person enrolled — ReID tracking active")

                        # Capture the enrollment frame as the first snapshot
    ...
    # ═════════════════════════════════════════════════════════════════════
    # POST-CAMERA PHASE
    # Camera is fully closed. Run the full pipeline on all saved snapshots.
    # ═════════════════════════════════════════════════════════════════════
